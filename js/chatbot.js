/* ============================================================
   ...
   - Model returns structured JSON so we can render inline
     "recommended course" cards (icon, fee, CTA) — not just text
   ============================================================ */
...
(function () {
  ...
  function buildSystemPrompt() {
    ...
    return `You are the "Corvit AI Counselor" — the official AI admissions counselor chatbot for ${CONFIG.INSTITUTE_NAME}, Pakistan's IT & Cisco training institute.
...
COURSES (id | name | category | duration | prerequisites | fee | hardware | placement | syllabus):
${courseLines}
...
"recommend" must be an array of course ids from the list above (use [] if no specific course applies to this reply).`;
  ...
  function addBotBubble(text, recommendIds) {
    const row = document.createElement("div");
    row.className = "chat-row chat-row-bot flex-col items-start gap-2.5";
    ...
    (recommendIds || []).forEach((id) => {
      ...
      card.className = "chat-course-card w-full max-w-[85%]";
      card.innerHTML = `
        <div class="w-11 h-11 shrink-0 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-xl">${course.icon}</div>
        <div class="flex-1 min-w-0">
          <div class="text-sm font-semibold text-white">${course.name}</div>
          <div class="text-xs text-slate-400 mt-0.5">${course.duration} · ${fmtPKR(course.feeMin)}${course.feeMin !== course.feeMax ? "–" + fmtPKR(course.feeMax) : ""}</div>
          <div class="flex gap-2 mt-2">
            <a href="#courses" class="chat-quick-chip">View course</a>
            <a href="#fee-calculator" class="chat-quick-chip">Fee calculator</a>
          </div>
        </div>`;
  ...
  function addTypingIndicator() {
    ...
    row.id = "typing-row";
    row.innerHTML = `<div class="chat-bubble chat-bubble-bot flex items-center gap-1.5"><span class="typing-dot"></span><span class="typing-dot"></span><span class="typing-dot"></span></div>`;
    messagesEl.appendChild(row);
    scrollToBottom();
  }
  ...
  function addErrorBubble(text) {
    const row = document.createElement("div");
    row.className = "chat-row chat-row-bot";
    row.innerHTML = `<div class="chat-bubble chat-bubble-bot border-red-500/30 text-red-300"></div>`;
    row.querySelector(".chat-bubble").textContent = text;
    messagesEl.appendChild(row);
    scrollToBottom();
  }

  /* ---------------- Groq API calls with fallback ---------------- */
  async function callGroqModel(model, messages) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Groq API error (${res.status}) on model ${model}: ${errText.slice(0, 200)}`);
    }
  ...
  async function getAssistantReply(userMessages) {
    const payload = [{ role: "system", content: buildSystemPrompt() }, ...userMessages];

    // Try primary model first, then fall back automatically.
    try {
      return await callGroqModel(CONFIG.MODEL_PRIMARY, payload);
    } catch (primaryErr) {
      console.warn("Primary model failed, trying fallback:", primaryErr.message);
      try {
        return await callGroqModel(CONFIG.MODEL_FALLBACK, payload);
      } catch (fallbackErr) {
  ...
  function parseModelJson(raw) {
    ...
    try {
      ...
      return { reply: obj.reply || cleaned, recommend: Array.isArray(obj.recommend) ? obj.recommend : [] };
    } catch (e) {
      // Model didn't return valid JSON — just show the raw text.
      return { reply: cleaned, recommend: [] };
    }
  }
  ...
  async function sendMessage(text) {
    ...
    if (!CONFIG.GROQ_API_KEY || CONFIG.GROQ_API_KEY === "PASTE_YOUR_GROQ_API_KEY_HERE") {
      addUserBubble(text);
      addErrorBubble("⚠️ The AI Counselor isn't configured yet — add your Groq API key in js/config.js to activate live chat. In the meantime, call our helpline +92 303 8888555 or browse the Courses section above.");
      input.value = "";
      return;
    }
    ...
    try {
      ...
      const { reply, recommend } = parseModelJson(raw);
      addBotBubble(reply, recommend);
      history.push({ role: "assistant", content: JSON.stringify({ reply, recommend }) });
    } catch (err) {
      removeTypingIndicator();
      addErrorBubble("Sorry, I'm having trouble connecting right now. Please try again in a moment, or call our helpline +92 303 8888555.");
      console.error(err);
    } finally {
      isSending = false;
  ...
  document.getElementById("nav-ask-ai").addEventListener("click", openPanel);
  document.getElementById("mobile-ask-ai").addEventListener("click", openPanel);
  ...
  window.openChatWithPrompt = function (promptText) {
    openPanel();
    setTimeout(() => sendMessage(promptText), 150);
  };
})();
